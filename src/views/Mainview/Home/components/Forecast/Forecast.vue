  ...
  <mdui-card style="position: relative; overflow: hidden; width: 100%">
    <mdui-top-app-bar ref="appBar">
      ...
      <mdui-dropdown stay-open-on-click>
        ...
        <mdui-menu
          selects="multiple"
          :value="sourceSelectsList"
          @change="sourceSelectsList = $event.target.value"
        >
          <span style="font-size: var(--mdui-typescale-label-large-size)"
            >数据源:</span
          >
          <mdui-menu-item
      ...
      <mdui-tabs>
        <template v-for="(item, index) in sourceList">
          <mdui-tab
            :key="index"
            :value="index.toString()"
            v-if="isSourceShow(index.toString())"
            >{{ item.name }}</mdui-tab
          >
        </template>
        <mdui-tab-panel
...
let sourceSelectsList = ref(
  JSON.parse(
    localStorage.getItem(instance!.type.name + "_sourceSelectsList") || '["0"]',
  ),
);
function isSourceShow(item?: string) {
  ...
  localStorage.setItem(
    instance!.type.name + "_sourceSelectsList",
    JSON.stringify(sourceSelectsList.value),
  );
  return sourceSelectsList.value.find((el: string) => el == item) != undefined
    ? true
    : false;
}
</script>
